...
public class ResidentService : IResidentService
{
	...
	public async Task<IEnumerable<ResidentResponseDto>> GetAllResidents(int managerId)
	{
		...
		.Select(a => new ResidentResponseDto
		{
			...
			Apartments = a.ApartmentResidents
			.Where(ar => ar.Status == GeneralStatus.Active &&
						 ar.IsDeleted == false &&
						 ar.Apartment.Building.ManagerId == managerId)
			.Select(ar => new ResidentApartmentDto
			{
				ApartmentId = ar.ApartmentId ?? 0,

				ApartmentCode = ar.Apartment.ApartmentCode,
				RelationshipId = ar.RelationshipId ?? 0,
				RelationshipName = ar.Relationship != null ? ar.Relationship.RelationshipName : "Thành viên"
			})
			.ToList()
		})
		.ToListAsync();
