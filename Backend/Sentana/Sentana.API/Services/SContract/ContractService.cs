...
namespace Sentana.API.Services
{
    public class ContractService : IContractService
    {
        ...
        public async Task<ApiResponse<object>> TerminateContractAsync(int contractId, TerminateContractDto request)
        {
            ...
            if (request.TerminationDate < contract.StartDay)
            ...
            if (request.AdditionalCost < 0)
                return ApiResponse<object>.Fail(400, "Chi phí thêm không hợp lệ.");
        ...
        public async Task<ApiResponse<object>> CreateContractAsync(CreateContractDto request, int accountId)
        {
            ...
            var account = await _contractRepository.GetAccountAsync(accountId);
        ...
        public async Task<ApiResponse<object>> UpdateContractAsync(int contractId, UpdateContractDto request)
        {
            ...
            if (request.File != null && request.File.Length > 0)
            {
                var latestVersion = await _contractRepository.GetLatestContractVersionAsync(contractId);

                decimal newVersion = latestVersion == null
                    ? 1.0m
        ...
        public async Task<ApiResponse<object>> GetContractListAsync()
        {
            ...
            var result = contracts.Select(c => new ContractDetailDto
            {
            ...
            return ApiResponse<object>.Success(result, "Lấy danh sách hợp đồng thành công.");
        }
    }
}